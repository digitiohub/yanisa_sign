const { deliver, getMailer } = require('../config/mailer');
...
async function sendSignatureRequest({ signer, document, url, waitingOnOthers = false }) {
  ...
  const template = waitingOnOthers ? 'final_approver_pending' : 'signature_request';
  return deliver({
    to: signer.email,
    subject: waitingOnOthers ? `Final approval pending – ${document.title}` : (document.subject || `Signature Request – ${document.title}`),
    html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto"><h2>${waitingOnOthers ? 'You are the final approver' : 'Signature request'}</h2>${body}<p><b>Document:</b> ${document.title}</p><p>Regards,<br>HR Team<br>Yanisa</p></div>`,
  }, { template, companyId: document.companyId, documentId: document._id, devOutbox: () => writeDevOutbox({ to: signer.email, template, subject: document.title, variables: { url } }) });
}
...
async function sendFinalApprovalReady({ signer, document, url, signedBy = [] }) {
  ...
  }, { template: 'final_approval_ready', companyId: document.companyId, documentId: document._id, devOutbox: () => writeDevOutbox({ to: signer.email, template: 'final_approval_ready', subject: document.title, variables: { url } }) });
}

// CC/BCC observers are told a request went out. Addressed to the sender's own
// address so the observer list stays in cc/bcc rather than being disclosed.
async function sendRequestObservers(document) {
  if (!document.cc?.length && !document.bcc?.length) return;
  const mailer = await getMailer();
  const to = mailer ? mailer.fromEmail : (document.createdBy || 'observers');
  return deliver({
    to, cc: document.cc || [], bcc: document.bcc || [],
    subject: `Signature request sent – ${document.title}`,
    html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto"><h2>Signature request sent</h2><p>The document <b>${document.title}</b> has been sent to ${document.signers.map(s => s.name).join(', ')} for signature.</p><p><b>Reference:</b> ${document.referenceNumber}</p><p>This notification does not contain a signer access link.</p></div>`,
  }, { template: 'request_observers', companyId: document.companyId, documentId: document._id });
}

const completionAttachments = document => [
  ...(document.signedFile ? [{ filename: `Signed - ${String(document.title).replace(/[^a-z0-9._-]/gi, '_')}.pdf`, path: document.signedFile, contentType: 'application/pdf' }] : []),
...
async function sendCompletion({ signer, document, notifyHr = false }) {
  const context = { companyId: document.companyId, documentId: document._id };
  await deliver({
    to: signer.email,
    ...
    attachments: completionAttachments(document),
  }, { template: 'completion', ...context });

  if (notifyHr && process.env.HR_NOTIFICATION_EMAIL) {
    await deliver({
      to: process.env.HR_NOTIFICATION_EMAIL,
      subject: `Signature request completed – ${document.title}`,
      html: `<p>All signers have completed <b>${document.title}</b>.</p><p>Reference: ${document.referenceNumber}</p>`,
      attachments: completionAttachments(document),
    }, { template: 'completion_hr', ...context });
  }
}
