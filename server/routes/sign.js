...
function diffDesign(previous, next) {
  ...
  for (const [id, field] of fieldsAfter) {
    ...
    const properties = candidate => ({ label: candidate.label, required: Boolean(candidate.required), readOnly: Boolean(candidate.readOnly), alignment: candidate.alignment || 'left', placeholder: candidate.placeholder || '' });
    const wasProperties = properties(before), isProperties = properties(field);
    if (Object.keys(isProperties).some(key => wasProperties[key] !== isProperties[key]))
      events.push({ action: 'field.updated', entityType: 'field', description: `changed the properties of the ${label(field)}`, before: wasProperties, after: isProperties });
  }
