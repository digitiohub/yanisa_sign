...
function PdfPages({ url, fields=[], witness=[], onDrop, onPages, onFieldClick, onFieldContext, onFieldChange, selected, interactive=false, values={}, onValue, zoom=1 }) {
  ...
  return <div className="space-y-6">{pages.map(p=><div key={p.number} className="pdf-page" style={{aspectRatio:`1/${p.ratio}`}} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const type=e.dataTransfer.getData('fieldType');if(!type||!onDrop)return;const r=e.currentTarget.getBoundingClientRect();onDrop(type,p.number,(e.clientX-r.left)/r.width,(e.clientY-r.top)/r.height)}}>
    <img src={p.url}/>
    {/* Already-signed values from other signers. Static: they exist so the
        final approver can verify what they are approving, never to be edited. */}
    {witness.filter(f=>f.pageNumber===p.number).map(f=>{
      const isImage=typeof f.value==='string'&&f.value.startsWith('data:image/');
      return <div key={`w-${f._id}`} className="witness-field" title={`Signed by ${f.signerName}`}
        style={{left:`${f.x*100}%`,top:`${f.y*100}%`,width:`${f.width*100}%`,height:`${f.height*100}%`,textAlign:f.alignment||'left'}}>
        {isImage?<img src={f.value} alt={`Signature of ${f.signerName}`}/>:<span>{f.value===true?'\u2713':String(f.value)}</span>}
        <em>{f.signerName}</em>
      </div>;
    })}
    {fields.filter(f=>f.pageNumber===p.number).map((f,i)=>{
      ...
      return <div key={f._id||i} data-field-type={f.type} onPointerDown={e=>startPointer(e,f)} onClick={()=>onFieldClick?.(f)} onContextMenu={e=>openContext(e,f)}
        ...
        {interactive
          ...
          : <>{glyphOnly ? <FieldIcon className="field-icon" size={14}/> : <span className="field-text">{f.placeholder||f.label||f.type}</span>}{onFieldChange&&<i title="Drag to resize" onPointerDown={e=>startPointer(e,f,'resize')}/>}</>}
      </div>;
    })}
  </div>)}</div>;
}
...
function Designer() {
  ...
  const addSigner=()=>setSignerModal({name:'',email:'',isFinalApprover:false});
  // Marking someone the final approver clears the flag everywhere else: two
  // approvers would each be waiting for the other, so the server rejects it.
  const saveSigner=form=>{const exclusive=(list,id)=>form.isFinalApprover?list.map(s=>({...s,isFinalApprover:String(s._id)===String(id)})):list;
    if(form._id){setSigners(exclusive(signers.map(s=>s._id===form._id?{...s,name:form.name,email:form.email,isFinalApprover:!!form.isFinalApprover}:s),form._id));setActiveSigner(form._id)}
    else{const signer={_id:newObjectId(),name:form.name,email:form.email,type:'Candidate',order:signers.length+1,status:'pending',isFinalApprover:!!form.isFinalApprover};setSigners(exclusive([...signers,signer],signer._id));setActiveSigner(signer._id)}
    setSignerModal(null);setDirty(true)};
...
function PublicSignV2(){
  ...
  const load=useCallback(()=>{setBusy(true);return api.get(`/sign/request/${token}`).then(({data})=>{setData(data);const initial={};data.fields.forEach(f=>{if(f.type==='name')initial[f._id]=data.signer.name;if(f.type==='email')initial[f._id]=data.signer.email;if(f.type==='phone')initial[f._id]=data.signer.phone||'';if(f.type==='date')initial[f._id]=new Date().toISOString().slice(0,10)});setValues(initial)}).catch(e=>setError(errorText(e))).finally(()=>setBusy(false))},[token]);
  useEffect(()=>{load()},[load]);
  // A held-back approver is waiting on other people, so the page checks for
  // itself rather than making them reload to find out.
  useEffect(()=>{if(!data?.waiting)return undefined;const timer=setInterval(load,30000);return()=>clearInterval(timer)},[data?.waiting,load]);
  ...
  return <div className="public-signing-page">
    ...
    {data.waiting&&<div className="approver-wait">
      <div className="approver-wait-head"><ShieldCheck size={18}/><div><b>You sign last on this document</b><span>Your signature completes it, so it unlocks once everyone else has signed. You can review the document and their signatures now.</span></div><button className="secondary-button" disabled={busy} onClick={load}>{busy?<Loader2 className="animate-spin" size={15}/>:<RefreshCw size={15}/>}Refresh</button></div>
      <ul className="approver-wait-list">{data.others.map(other=><li key={other.id} className={other.status==='completed'?'done':other.status==='declined'?'declined':''}>
        {other.status==='completed'?<Check size={15}/>:other.status==='declined'?<X size={15}/>:<CircleDot size={15}/>}
        <div><b>{other.name}</b><small>{other.email}</small></div>
        <span>{other.status==='completed'?`Signed ${formatDate(other.completedAt)}`:other.status==='declined'?'Declined':other.status==='viewed'?'Opened, not signed':'Not opened'}</span>
      </li>)}</ul>
    </div>}
    {!started&&!data.waiting&&<button className="click-to-start" onClick={()=>{setStarted(true);document.querySelector('.public-pdf-area .interactive')?.scrollIntoView({behavior:'smooth',block:'center'})}}>CLICK TO START</button>}
    <main className="public-pdf-area" style={{width:`${Math.max(820,1080*zoom)}px`}}>{error&&<div className="error-box">{error}</div>}<PdfPages url={`/api/sign/request/${token}/pdf`} fields={data.fields} witness={data.witnessFields||[]} interactive={!data.waiting} zoom={zoom} values={values} onValue={(f,v)=>{setValues({...values,[f._id]:v});setStarted(true)}}/></main>
    {data.waiting&&<div className="public-finish-card locked"><span className="finish-locked-note"><Lock size={15}/>Your signature is the last one. The button unlocks once everyone else has signed.</span><button disabled className="primary-button">Finish signing</button><button disabled={busy} onClick={decline}>Decline to sign</button></div>}
    {started&&!data.waiting&&<div className="public-finish-card"><label><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/><span>I confirm that I have reviewed this document and agree to use my electronic signature for this document.</span></label><button disabled={!consent||busy} onClick={complete} className="primary-button">{busy&&<Loader2 className="animate-spin" size={16}/>}Finish signing</button><button disabled={busy} onClick={decline}>Decline to sign</button></div>}
  </div>
}

function SignerModal({signer,onClose,onSave}) { const [form,setForm]=useState({...signer}),[error,setError]=useState(''); const submit=e=>{e.preventDefault();const name=form.name.trim(),email=form.email.trim().toLowerCase();if(!name)return setError('Enter the signer name.');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return setError('Enter a valid email address.');onSave({...form,name,email})}; return <div className="modal-backdrop"><motion.form initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} onSubmit={submit} className="modal max-w-md"><div className="flex items-center justify-between"><div><p className="eyebrow">Signer details</p><h2 className="text-xl font-semibold">{signer._id?'Edit signer':'Add signer'}</h2></div><button type="button" onClick={onClose}><X/></button></div><p className="mt-2 text-sm text-slate-500">Add the person who needs to complete fields on this document.</p>{error&&<div className="error-box">{error}</div>}<label className="field-label">Name<input autoFocus value={form.name} placeholder="Signer 1" onChange={e=>setForm({...form,name:e.target.value})}/></label><label className="field-label">Email address<input type="email" value={form.email} placeholder="signer@example.com" onChange={e=>setForm({...form,email:e.target.value})}/></label><label className="approver-toggle"><input type="checkbox" checked={!!form.isFinalApprover} onChange={e=>setForm({...form,isFinalApprover:e.target.checked})}/><span><b>Final approver</b><small>Signs last. Their link stays locked until everyone else has signed, and they can review the other signatures before approving.</small></span></label><div className="mt-6 flex justify-end gap-2"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button">{signer._id?'Save changes':'Add signer'}</button></div></motion.form></div> }
